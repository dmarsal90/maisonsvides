...
class EstateController extends Controller {
	...
	public function setRdv(Request $request) {
		...
		$updated = $this->updateData(app("App\\Models\\Estate"), 'category', 3, $data['estate_id'], $data['estate_id'], 'category'); // Update data
		$estates = $this->getEstates();
		// If session is of the a manager
		if (Auth::user()->type == 2) {
			// Get estates to show to manager
		...
		foreach ($estates as $estate) {
			if (empty($estate['offre'])) {
				$updated = $this->updateData(app("App\\Models\\Estate"), 'category', 4, $estate['offre'], $estate['offre'], 'category'); // Update data
			}
		}
		return response($response)->header('Content-Type', 'application/json');
	}
	...
	public function editDetails(Request $request) {
		...
		foreach($data as $key => $dat) { // Foreach key of data
			...
			if ($key == 'estate_description') {
				EstateDetail::where('estate_id', '=', $data['estate_id'])->update(['description' => $dat]);
			}
		...
		try {
			...
			$response = array(
				'status' => false, // Reponse status
				'message' => $e->getMessage() // Response message
			);
		}
